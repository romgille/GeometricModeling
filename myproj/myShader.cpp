...
{
	init(file_vertexshader, file_fragmentshader);
}

myShader::myShader()
	:vertex_shader(0), fragment_shader(0), geometry_shader(0), shaderprogram(0)
{
}
...
void myShader::init(string file_vertexshader, string file_fragmentshader)
{
	text_to_id.clear();
	clear();
...
GLuint myShader::_initShader(GLenum type, string filename)
{
	...
	glShaderSource(shader, 1, &shadertext_cstr, nullptr);
